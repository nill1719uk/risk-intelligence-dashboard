...
st.set_page_config(page_title="AML Detection", layout="wide")
...
@st.cache_data
def load_alerts():
    return pd.read_csv("data/aml_alerts.csv", encoding="utf-8")
...
filtered = df if alert_filter == "ALL" else df[df["alert_confidence"] == alert_filter]
